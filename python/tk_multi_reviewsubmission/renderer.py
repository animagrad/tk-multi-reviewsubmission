...
from amg.system import navigator, image_convert
...
class Renderer(object):
    ...
    def render_movie_in_nuke(self, path, output_path, 
        ...
        group.begin()
        try:
            # Converting a render sequence from exr to png
            scene_path = nuke.root().name()
            fields = navigator.fields_from_path(scene_path)
            exr_path = navigator.path_from_fields(
                fields, data_type='compose', update={'frame': '%04d'})
            # img_dir = os.path.dirname(exr_path)
            # for f in os.listdir(img_dir):
            #     img_path = os.path.join(img_dir, f)
            #     image_convert.make_png(img_path)
            #     print img_path
            # color_space = 'default (ACES - ACEScg)'
            # create read node
            read = nuke.nodes.Read(name="source", file=path.replace(os.sep, "/")) # .replace('.exr', '.png')
            ...
            # output_node['colorspace'] = 'Output - sRGB'
